...
- [x] **Task 1.7.3:** Implement the `POST /auth/token` endpoint that:
...
- [x] **Task 1.8.1:** Implement `get_current_user(token: str = Depends(oauth2_scheme)) -> User` in `app/api/v1/auth.py` (or `dependencies.py`). This function should:
...
- [x] **Task 1.8.2:** Implement `get_current_active_user(current_user: User = Depends(get_current_user)) -> User`. This dependency should check `current_user.is_active`.

## Epic 2: Phase 2 - Core Domain Entities (Lodge & Rooms)
**Concept Focus:** SQLAlchemy Foreign Keys, UniqueConstraints, Pydantic Nested Schemas.

### 📝 Pending Tasks
- [x] **Task 2.1:** Create `Lodge` SQLAlchemy Model (`app/models/lodge.py`).
- [ ] **Task 2.2:** Create `Room` Pydantic Schemas (`app/schemas/room.py`).
- [ ] **Task 2.3:** Build `Room` CRUD operations (`app/crud/room.py`).
- [ ] **Task 2.4:** Build `Room` API Router (`app/api/v1/rooms.py`).
